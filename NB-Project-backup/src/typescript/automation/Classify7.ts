...
// Example:
import {
  SeptaClass, ///from current folder of /src/typscript/contract
  SeptaClassTransactionReceipt  //classes from the generated typscript wrapper
} from '../contracts/SeptaClass'
import { Eth } from 'web3x-es/eth'   //for interacting blockchain
const septaClassCreator: ContractCreator<SeptaClass> = (eth: Eth, address?: Address) => {
  try {
    return Promise.resolve(
      new SeptaClass(eth, address)
    )
  } catch (e) {
    return Promise.reject(e)
  }
}
const septaClassCache: ContractCache<SeptaClass> = ContractCache.getInstance(
  'SeptaClass', septaClassCreator
)
let inst: SeptaClass | undefined   ///What does this line mean
myDebugger('through SeptaClass')
...
ethereumRequestManager.discoverCorrectSendMethod().then(() => { // method to keep poking hdwallet to find out what is the send method and with promise
  ...
  return septaClassCache.getSendingAddress()
}).then((_addr: Address) => {
  myDebugger('got address %o', _addr)
  sendingAddress = _addr
  return septaClassCache.getOrCreateContractForCurrentNetwork()
}).then((_inst: SeptaClass) => {
  // do something with instance of contract
  myDebugger('got SeptaClass instance')
  inst = _inst
 ...
 }).then((rcpt: SeptaClassTransactionReceipt) => {
  // print output
  ...
  const x1 = splitCol6[0]
  const x2 = splitCol6[1]
  const x3 = splitCol6[2]
  const x4 = splitCol6[3]
  const v1 = splitCol6[4]
  const v2 = splitCol6[5]
  const v3 = splitCol6[6]
  const v4 = splitCol6[7]
  const m1 = splitCol6[8]
  const m2 = splitCol6[9]
  const m3 = splitCol6[10]
  const m4 = splitCol6[11]

  // send
  myDebugger('send7 time %o', getMostPreciseTimeInSecondsSinceUnixEpoch())
  start = getMostPreciseTimeInSecondsSinceUnixEpoch()
  return inst.methods.b_getparameters6(
    x1,x2,x3,x4,v1,v2,v3,v4,m1,m2,m3,m4
  ).send({
    from: sendingAddress,
    gas: 1000000
  ...
  appendFile(gasUsageFile, '\n')
  // get latencies
  splitCol7= splitRows[7].split(',')
  const prior0 = splitCol7[0]
  const prior1 = splitCol7[1]
  const prior2 = splitCol7[2]
  const prior3 = splitCol7[3]
  const prior4 = splitCol7[4]
  const prior5 = splitCol7[5]
  const prior6 = splitCol7[6]
  // send
  myDebugger('send8 time %o', getMostPreciseTimeInSecondsSinceUnixEpoch())
  start = getMostPreciseTimeInSecondsSinceUnixEpoch()
  ...
  appendFile(timeFile, String(diff))
  appendFile(timeFile, '\n')
  myDebugger('recv8 time %o', getMostPreciseTimeInSecondsSinceUnixEpoch())
  //myDebugger('recv8 rcpt %o', rcpt)
  appendFile(gasUsageFile, String(rcpt.gasUsed))
  appendFile(gasUsageFile, '\n')
  // perform call
