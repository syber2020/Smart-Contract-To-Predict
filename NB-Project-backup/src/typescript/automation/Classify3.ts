...
// Example:
import {
  TripleClass, ///from current folder of /src/typscript/contract
  TripleClassTransactionReceipt  //classes from the generated typscript wrapper
} from '../contracts/TripleClass'
import { Eth } from 'web3x-es/eth'   //for interacting blockchain
const tripleClassCreator: ContractCreator<TripleClass> = (eth: Eth, address?: Address) => {
  try {
    return Promise.resolve(
      new TripleClass(eth, address)
    )
  } catch (e) {
    return Promise.reject(e)
  }
}
const tripleClassCache: ContractCache<TripleClass> = ContractCache.getInstance(
  'TripleClass', tripleClassCreator
)
let inst: TripleClass | undefined   ///What does this line mean
myDebugger('through TripleClass')
...
ethereumRequestManager.discoverCorrectSendMethod().then(() => { // method to keep poking hdwallet to find out what is the send method and with promise
  ...
  return tripleClassCache.getSendingAddress()
}).then((_addr: Address) => {
  myDebugger('got address %o', _addr)
  sendingAddress = _addr
  return tripleClassCache.getOrCreateContractForCurrentNetwork()
}).then((_inst: TripleClass) => {
  // do something with instance of contract
  myDebugger('got TripleClass instance')
  inst = _inst
...
}).then((rcpt: TripleClassTransactionReceipt) => {
  // print output
  ...
  const prior0 = splitCol3[0]
  const prior1 = splitCol3[1]
  const prior2 = splitCol3[2]

  // send
  myDebugger('send4 time %o', getMostPreciseTimeInSecondsSinceUnixEpoch())
  start = getMostPreciseTimeInSecondsSinceUnixEpoch()
  return inst.methods.e_exponent1(
