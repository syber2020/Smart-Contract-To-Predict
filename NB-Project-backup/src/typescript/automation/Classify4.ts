...
// Example:
import {
  QuadClass, ///from current folder of /src/typscript/contract
  QuadClassTransactionReceipt  //classes from the generated typscript wrapper
} from '../contracts/QuadClass'
import { Eth } from 'web3x-es/eth'   //for interacting blockchain
const quadClassCreator: ContractCreator<QuadClass> = (eth: Eth, address?: Address) => {
  try {
    return Promise.resolve(
      new QuadClass(eth, address)
    )
  } catch (e) {
    return Promise.reject(e)
  }
}
const quadClassCache: ContractCache<QuadClass> = ContractCache.getInstance(
  'QuadClass', quadClassCreator
)
let inst: QuadClass | undefined   ///What does this line mean
myDebugger('through QuadClass')
...
ethereumRequestManager.discoverCorrectSendMethod().then(() => { // method to keep poking hdwallet to find out what is the send method and with promise
  ...
  return quadClassCache.getSendingAddress()
}).then((_addr: Address) => {
  myDebugger('got address %o', _addr)
  sendingAddress = _addr
  return quadClassCache.getOrCreateContractForCurrentNetwork()
}).then((_inst: QuadClass) => {
  // do something with instance of contract
  myDebugger('got QuadClass instance')
  inst = _inst
 ...
 }).then((rcpt: QuadClassTransactionReceipt) => {
  // print output
  end = getMostPreciseTimeInSecondsSinceUnixEpoch()
  diff = end - start
  //myDebugger('recv1 rcpt %o', rcpt)
  ...
  const prior0 = splitCol4[0]
  const prior1 = splitCol4[1]
  const prior2 = splitCol4[2]
  const prior3 = splitCol4[3]

  // send
  myDebugger('send5 time %o', getMostPreciseTimeInSecondsSinceUnixEpoch())
  start = getMostPreciseTimeInSecondsSinceUnixEpoch()
  return inst.methods.test(
    prior0,prior1,prior2,prior3
  ).send({
    from: sendingAddress,
    gas: 1000000
  }).getReceipt()
