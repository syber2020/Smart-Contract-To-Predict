...
// Example:
import {
  PentaClass, ///from current folder of /src/typscript/contract
  PentaClassTransactionReceipt  //classes from the generated typscript wrapper
} from '../contracts/PentaClass'
import { Eth } from 'web3x-es/eth'   //for interacting blockchain
const hexaClassCreator: ContractCreator<PentaClass> = (eth: Eth, address?: Address) => {
  try {
    return Promise.resolve(
      new PentaClass(eth, address)
    )
  } catch (e) {
    return Promise.reject(e)
  }
}
const hexaClassCache: ContractCache<PentaClass> = ContractCache.getInstance(
  'PentaClass', hexaClassCreator
)
let inst: PentaClass | undefined   ///What does this line mean
myDebugger('through PentaClass')
...
ethereumRequestManager.discoverCorrectSendMethod().then(() => { // method to keep poking hdwallet to find out what is the send method and with promise
...
}).then((_inst: PentaClass) => {
  // do something with instance of contract
  myDebugger('got PentaClass instance')
  inst = _inst
 ...
 }).then((rcpt: PentaClassTransactionReceipt) => {
  // print output
  ...
  const prior0 = splitCol5[0]
  const prior1 = splitCol5[1]
  const prior2 = splitCol5[2]
  const prior3 = splitCol5[3]
  const prior4 = splitCol5[4]
  // send
  myDebugger('send6 time %o', getMostPreciseTimeInSecondsSinceUnixEpoch())
  start = getMostPreciseTimeInSecondsSinceUnixEpoch()
  return inst.methods.test(
    prior0,prior1,prior2,prior3,prior4
  ).send({
    from: sendingAddress,
    gas: 1000000
  }).getReceipt()
